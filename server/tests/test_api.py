...
def test_v1_environment(endpoint="/environment") -> None:
...
def test_v1_users_get_all(endpoint="/users") -> None:
...
def test_v1_users_postone_missingparams(endpoint="/users") -> None:
...
def test_v1_users_postone_unmatchparams(endpoint="/users") -> None:
...
def test_v1_users_postone_goodcase(endpoint="/users") -> None:
...
def test_v1_users_postone_conflictonexisting(endpoint="/users") -> None:
...
def test_v1_users_postone_newonewithtokens(endpoint="/users") -> None:
    pp.pprint("Test POST one user - good parameters with tokens")
    r= requests.post(core_url+ "/users?withToken=true", headers = h, json={"display_name":"john", "password":"yerblues","confirm_password":"yerblues","phone_number":"+33687853133"})
    if r.status_code != 201 and r.status_code != 200:
        pp.pprint("  ->Test FAILED")
        pp.pprint(r.json())
    else:
...
def test_v1_users_getone_nonexisting(endpoint="/users") -> None:
...
def test_v1_users_gettone_existingcore(endpoint="/users") -> None:
...
def test_v1_users_getonetokens(endpoint="/users") -> None:
    #pp.pprint("Test GET one user: existing")
    pp.pprint("Test GET all tokens: not existing")
    r=requests.get(core_url+ "/users/"+user_uuid)
    if r.status_code != 200:
...
def test_v1_users_deleteone_nonexisting(endpoint="/users") -> None:
    #pp.pprint("Test GET one user: existing")
    pp.pprint("Test GET all tokens: not existing")
    r=requests.get(core_url+ "/users/"+user_uuid)
    if r.status_code != 404:
...
def test_v1_users_deleteone_existing(endpoint="/users") -> None:
    #pp.pprint("Test GET one user: existing")
    pp.pprint("Test GET all tokens: not existing")
    r=requests.get(core_url+ "/users/"+user_uuid)
    if r.status_code != 200:
